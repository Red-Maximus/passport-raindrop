...
RaindropStrategy.prototype.authenticate = async function(req) {
    ...
    await strat._verify(req.session.user, async (error, user) => {
        ...
        let hydroRes = await strat.partner.verifySignature(user.hydroId, req.session.hydroMessage).catch(err => { return strat.error(err) });

        // Pass or fail the strategy based on the return object
        return hydroRes && hydroRes.verified ? strat.success(user) : strat.fail();
